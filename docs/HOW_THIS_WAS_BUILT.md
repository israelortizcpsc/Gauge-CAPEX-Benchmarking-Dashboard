...
![Gauge dashboard](screenshots/dashboard.png)
...
![Filtering the peer group](screenshots/filter.gif)
...
![Benchmarking individual projects](screenshots/project.gif)
...
materialized into a filter-keyed cache. The full architecture and API are in the
[README](../capbench-README.md); the engineering decisions are in
...
- **Scaffolding.** Standing up the Django project, the DRF viewset/serializer
  wiring, the Vite app, the env-driven settings, the Dockerfile and Render
  blueprint — the boilerplate I've written a dozen times. Letting AI type it
  meant I spent my time on the model and the math instead of `startproject`.
- **Tests.** It drafted the bulk of the 37-test suite. I directed *what* to
  test (percentile edge cases, role-scoped reads, peer-group widening) and
  reviewed every assertion, but the typing-out was AI.
- **Rubber-ducking the percentile math.** I wanted the percentile function to
  ...
  approximation that's subtly off. Talking through the interpolation method —
  and pinning it with tests against NumPy's default `linear` method — was faster
  as a dialogue than alone.
...
- **The data model is mine.** What a `Project` is, which dimensions define a peer
  ...
  *intensity* before it's compared, why the FEL score weights scope above
  ...
  and I made them.
- **The stats layer is mine.** Everything in
  [`stats.py`](../backend/benchmarks/stats.py) — percentiles, percentile rank
  with a midpoint tie convention, the cost-gap definition, the FEL roll-up — I
  can derive on a whiteboard. It's deliberately framework-free so it's the part
  that's easiest to reason about and hardest to fake understanding of.
- **The architecture is mine.** The decision to compute benchmarks *server-side*
  for a single source of truth, to materialize aggregates keyed by an
  order-independent filter signature, to widen the peer group when a slice is too
  thin — those are the load-bearing choices, and I can defend each trade-off.

My rule: **if I couldn't explain a line, I rewrote it until I could.** A few of
the AI's first drafts were clever in ways I didn't want to inherit; I'd rather
ship something slightly plainer that I fully understand.
...
The benchmark service caches peer-group results in Django's in-process cache.
The AI's test suite passed when I ran tests one at a time. Run the *whole* file,
and a service test started failing: it expected a freshly-seeded peer group of
**2 projects** but got **20**.
...
aggregate with 20 projects and left it sitting in the cache. The next test, with
its own 2 projects, asked for `sector=mining`, got a cache hit, and read the
*previous* test's stale answer. The AI had quietly assumed the cache rolled back
like the DB. It doesn't.
...
the AI wrote a bug (everyone writes bugs), but that I could read its code well
enough to know *why* it was wrong before trusting it.
